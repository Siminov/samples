...
if(window['document'] == undefined) {
    ...
    var ReactInterceptor = require('NativeModules').ReactInterceptor;
...
Adapter.invoke = function(adapter) {
     ...
     */
    if(SIHReactInterceptor || ReactInterceptor) {
        Log.debug("Adapter", "invoke", "handleHybridToNativeAsync: REQUEST ID: " + adapter.getRequestId() + ", ADAPTER NAME: " + adapterName + ", HANDLER NAME: " + handlerName + ", DATA: " + json);
        
        var interceptor;
        if(SIHReactInterceptor) {
            interceptor = SIHReactInterceptor;
        } else if(ReactInterceptor) {
            interceptor = ReactInterceptor;
        }
        
        if(adapterMode == undefined || adapterMode == null || adapterMode == Adapter.ADAPTER_SYNC_MODE) {
            interceptor.handleHybridToNative(Constants.HTTP_POST_METHOD, adapterName + "." + handlerName, json, reactSyncCallback);
            ...
            interceptor.handleHybridToNativeAsync(Constants.HTTP_POST_METHOD, adapter.getRequestId(), adapterName + "." + handlerName, json);
        }
